...
- **多设备管理** — 同时模拟多个设备并发接入，支持一键批量连接除 Video 外的可恢复协议设备
- **设备编辑** — 支持直接编辑已有模拟设备，保存时只更新配置字段
- **批量导入/导出** — JSON/CSV 文件批量导入设备，一键导出当前设备配置
- **场景编排** — 按连接、等待、发送、断开步骤批量执行，通用发送步骤覆盖 HTTP / MQTT / CoAP
- **自定义协议验证** — 在模拟器内直接加载当前设备可用规则，执行上行调试、下行编码，并辅助做真实运行态联调
- **压力测试** — 面向 HTTP / MQTT / CoAP 通用上报链路执行并发 N 台设备×M 轮发送，实时统计 TPS/成功率/耗时
- **配置持久化** — 设备配置和自定义模板自动保存到 localStorage，刷新后恢复
- **实时日志** — 查看所有设备的连接、发送、错误日志，统一记录 MQTT 下行 payload 和断连/异常事件
