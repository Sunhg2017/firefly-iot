...
- 视频设备先区分 `GB28181`、`RTSP`、`RTMP`，再分别展示对应配置。
...
## 12. 2026-03-26 视频设备联调补充

### 12.1 目标

- Video 模式不再只做“本地表单配置”，而是直接对齐平台当前视频设备链路。
- 模拟器 Video 协议与产品协议、平台视频设备、视频控制接口统一收口到同一套标识口径。
- 再次连接视频设备时优先复用平台已有记录，避免联调过程中反复造重复数据。

### 12.2 关键设计

- 视频模式统一为 `GB28181 / RTSP / RTMP`。
- Video 设备连接、更新、推流、截图、录制、目录查询、设备信息查询统一复用当前环境登录态，并通过当前环境 `gatewayBaseUrl` 的 `/MEDIA/api/**` 路由访问视频接口。
- `GB28181` 继续使用 `gbDeviceId` 作为设备业务标识与本地 `DeviceName`。
- `RTSP / RTMP` 使用完整 `sourceUrl` 作为平台侧主标识，同时自动解析 `IP / 端口` 用于地址展示和兼容旧接口。
- 模拟器连接时按以下顺序同步平台视频设备：
  1. 本地已有 `videoDeviceId` 时，直接更新对应平台设备。
  2. 本地没有 `videoDeviceId` 时，先按接入标识查询平台现有设备。
  3. 若平台已存在同一标识，则转为更新；否则创建新设备。

### 12.3 兼容边界

- 本地导入和旧缓存中如果仍出现 `RTSP_PROXY / rtspUrl`，仅在导入阶段做一次性迁移到 `RTSP / sourceUrl`，运行时不再保留双轨分支。
- Video 设备仍不参与启动自动恢复；但手工重新连接时已经具备平台侧幂等同步能力。
## 9. 2026-03-14 增补
