...
#### 3.4.1 2026-04-02 编辑抽屉易用性收口

本次对协议解析编辑抽屉继续做“降低误配成本”的收口，不改后端接口和步骤模型，只强化编辑过程中的即时反馈：

- 抽屉宽度扩大到 `1280px`，让 JSON、脚本与摘要信息可以并排查看，减少来回滚动。
- 抽屉右侧新增固定摘要区，统一展示：
  - 步骤完成进度
  - 当前步骤是否已满足继续条件
  - 当前规则关键摘要
- 右侧步骤卡片允许返回已到达步骤，用户可以直接从摘要区跳回前面补字段，而不需要只依赖顶部步骤条。
- `matchRuleJson`、`frameConfigJson`、`parserConfigJson`、`visualConfigJson`、`releaseConfigJson` 全部接入“必须是合法 JSON 对象”的前置校验，尽量把错误拦截在当前步骤，而不是等到最终保存。
- 右侧“当前步骤检查”提供“一键格式化 JSON / 校验本步”，减少因为缩进、括号或空白字符造成的低级错误。
- 预览确认步骤不再只显示静态内容；当用户已经进入预览页但前面步骤被改坏时，侧栏会明确提示“请先完成哪一步”，避免出现“看起来能保存、实际又会失败”的认知落差。
...
- 编辑抽屉与全屏编辑只保留当前操作直接需要的提示；模板背景、字段解释和联动细节统一收口到文档，不再占用编辑区空间。
...
- 预览确认页与右侧“当前摘要”新增直接回跳能力；用户在复核时发现问题，可以直接返回对应步骤或打开对应配置的全屏编辑，而不需要自己重新定位字段。
- 脚本区补充“当前方向骨架 / 按可视化重建”，JSON 全屏窗口补充“恢复推荐值”，让用户可以在写乱后快速回到当前上下文下的推荐配置。
...
3. 编辑步骤实时就绪判断
   - 右侧摘要区不会直接读取表单校验状态，而是根据当前可见字段和 JSON 解析结果实时计算“步骤是否就绪”。
   - 预览确认步骤的就绪状态取决于前四步是否全部满足条件，因此需要把“前置步骤未完成”显式折叠成可读提示，而不是只返回空结果。
