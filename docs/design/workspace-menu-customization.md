...
- 当租户尚未被系统运维分配任何租户空间菜单时，不应出现“菜单配置”入口或对应权限。
...
- `menu-customization` 作为租户菜单自定义入口，只在租户已有显式业务菜单授权时自动附带，禁止在“零授权”租户上继续保底放开。
