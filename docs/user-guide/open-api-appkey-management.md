...
- `组织与权限 -> AppKey 管理 -> 接口文档`
- 页面右上角 `接口文档`

页面内容：

- 只展示租户联调真正需要的信息，例如网关调用地址、HTTP 方法、参数字段、响应字段、请求示例、响应示例和 curl 示例。
- 不展示下游服务路径、权限透传编码等系统内部实现信息。
- 如果某个服务的接口详情还在同步中，页面会给出通用提示，稍后刷新即可。

## 7. 常见问题

### 7.1 新建 AppKey 时没有可选 OpenAPI
...
### 7.5 系统菜单管理里看不到 OpenAPI 管理或 AppKey 管理
