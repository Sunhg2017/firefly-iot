...
## 15. 2026-03-22 顶部区域移除补充

- 本次继续调整 `firefly-simulator/src/App.tsx`，移除页面内部独立应用头，让工作区直接从首屏顶部开始布局。
- 发布后如果仍看到顶部 `IoT / 设备模拟器 / 设备联调工作台` 卡片，优先确认是否仍在加载旧版 renderer 资源。
- 回归时重点检查：首屏不再出现单独头部占位，三栏工作区从页面顶部直接开始，页面外层仍保持无滚动。

## 16. 2026-03-22 主工作区信息卡合并补充

- 本次调整 `firefly-simulator/src/components/DeviceControlPanel.tsx`，移除独立“接入概览”卡，并将接入参数并入顶部基本信息卡。
- 同时为顶部基本信息卡增加主工作区内吸顶效果，滚动物模型 OpenAPI、数据上报和协议专属工具时，该信息卡应始终停留在最上方。
- 回归时重点检查：主工作区不再出现单独“接入概览”卡；`Broker`、认证方式、`ProductKey`、`DeviceName` 等字段集中显示在顶部信息卡；主工作区滚动时顶部信息卡不应随内容一起滚出视口。
