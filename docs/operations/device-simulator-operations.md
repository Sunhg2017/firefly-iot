...
3. 若日志出现 `SIP 提示：本地码流告警: ... Selected pixel format ...`，以及后续 `Supported pixel formats` 列表明细，说明系统已识别到输入像素格式不兼容，会继续按 `Supported pixel formats` 自动重试
4. 若日志继续出现 `SIP 提示：检测到摄像头参数不兼容，已切换设备默认采集模式重试`，说明已进入第二阶段降级，可继续观察是否恢复推流
5. 检查模拟器摄像头权限是否已授权
...
3. 若日志出现 `Selected pixel format ... is not supported`，优先观察后续是否已有 `改用 ... 重试` 或 `已切换设备默认采集模式重试` 提示
4. 检查媒体服务是否可达自动生成地址中的主机和端口
