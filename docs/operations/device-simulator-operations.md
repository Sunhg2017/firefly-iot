...
- Video 模式统一支持 `GB28181 / RTSP / RTMP`，并改为复用当前环境登录态同步平台视频设备。
- Video 模式的控制与同步接口统一通过当前环境网关 `/MEDIA/api/**` 调用。
...
- Video：
  - RTSP / RTMP 模式只要求完整源地址，并自动解析平台接入地址
  - GB28181 模式显示 SIP、设备信息和通道配置
  - 连接前必须先登录当前环境
...
### 5.6 Video 设备连接时报“请先登录当前环境”

排查顺序：

1. 确认左上角当前环境已登录。
2. 确认登录的就是目标联调租户，而不是别的环境。
3. 若刚切换过环境，重新执行一次连接，确认没有复用旧 token。
4. 若是控制区按钮报错，确认当前环境 `gatewayBaseUrl` 能访问 `/MEDIA/api/**`，且网关已正确转发到 `firefly-media`。

### 5.7 RTSP / RTMP 同主机不同路径的设备同步异常

排查顺序：

1. 确认第二步填写的是完整 `sourceUrl`，而不是只有主机和端口。
2. 确认 `firefly-media` 已执行 `V4__add_video_device_source_url.sql` 和 `V5__refine_video_device_proxy_identity_unique.sql`。
3. 如历史环境里残留了没有 `sourceUrl` 的旧 RTSP / RTMP 记录，先清理旧测试数据，再重新联调。
