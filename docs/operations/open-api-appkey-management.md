...
- `OpenAPI订阅` 抽屉支持按关键字、所属服务、HTTP 方法、订阅状态筛选，并支持按当前筛选结果批量订阅和批量取消
- 租户空间出现 `AppKey 管理`
...
- 进入租户空间 `/app-key?tab=docs` 后，确认页面不再展示下游路径、透传权限编码和面向研发的 Swagger 补注解提示。
- 若某个服务的快照尚未准备完成，页面只应提示“接口详情同步中”，不应直接暴露内部同步失败或解析失败细节。
- 右上角“接口文档”入口应与通知、告警、导出动作保持同一套轻量头部样式，不再表现为独立强调按钮。
