...
```
1. 管理员创建视频设备记录（GB28181 / RTSP / RTMP）
2. GB28181 设备通过 SIP 注册 → 自动更新设备状态为 ONLINE
3. 用户请求实时播放 → 平台调用 ZLMediaKit API 发起点播
4. ZLMediaKit 返回播放地址 → 平台保存流会话并返回前端
5. 前端使用 FLV.js / HLS.js 播放视频
6. 用户停止播放 → 平台调用 ZLMediaKit 关闭流 → 更新会话状态
```
