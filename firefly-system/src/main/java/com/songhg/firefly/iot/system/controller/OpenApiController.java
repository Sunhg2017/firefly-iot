...
public class OpenApiController {
    ...
    @PostMapping
    @RequiresPermission("openapi:create")
    @Operation(summary = "Create OpenAPI")
    public R<OpenApiVO> create(@Valid @RequestBody OpenApiCreateDTO dto) {
        userDomainService.assertCurrentUserIsSystemOps();
        return R.ok(openApiCatalogService.createOpenApi(dto));
    }

    @PutMapping("/{code}")
    @RequiresPermission("openapi:update")
    @Operation(summary = "Update OpenAPI")
    public R<OpenApiVO> update(@Parameter(description = "OpenAPI code", required = true) @PathVariable String code,
                               @Valid @RequestBody OpenApiUpdateDTO dto) {
        userDomainService.assertCurrentUserIsSystemOps();
        return R.ok(openApiCatalogService.updateOpenApi(code, dto));
    }

    @DeleteMapping("/{code}")
    @RequiresPermission("openapi:delete")
    @Operation(summary = "Delete OpenAPI")
    public R<Void> delete(@Parameter(description = "OpenAPI code", required = true) @PathVariable String code) {
        userDomainService.assertCurrentUserIsSystemOps();
        openApiCatalogService.deleteOpenApi(code);
        return R.ok();
    }
}
