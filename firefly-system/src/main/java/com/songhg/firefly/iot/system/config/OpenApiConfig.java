    ...
    public OpenAPI openAPI() {
        return new OpenAPI()
                ...
                .components(new Components()
                        ...
                        .addSecuritySchemes("appTimestampAuth", new SecurityScheme()
                                .type(SecurityScheme.Type.APIKEY)
                                .in(SecurityScheme.In.HEADER)
                                .name("X-Timestamp")
                                .description("Epoch milliseconds used in canonical signature"))
                        .addSecuritySchemes("appNonceAuth", new SecurityScheme()
                                .type(SecurityScheme.Type.APIKEY)
                                .in(SecurityScheme.In.HEADER)
                                .name("X-Nonce")
                                .description("Unique nonce to prevent replay"))
                        .addSecuritySchemes("appSignatureAuth", new SecurityScheme()
                                .type(SecurityScheme.Type.APIKEY)
                                .in(SecurityScheme.In.HEADER)
                                .name("X-Signature")
                                .description("HMAC-SHA256 signature over canonical request")))
