...
import { asyncTaskApi, deviceApi, deviceTagApi, fileApi, productApi } from '../../services/api';
...
interface DeviceCreateFormValues { productId: number; deviceName: string; nickname?: string; description?: string; tagIds?: number[]; }
interface DeviceBatchCreateFormValues { productId: number; description?: string; tagIds?: number[]; }
interface DeviceUpdateFormValues { nickname?: string; description?: string; tagIds?: number[]; }
...
const statusLabels: Record<string, string> = { INACTIVE: '未激活', ACTIVE: '已激活', DISABLED: '已禁用' };
const statusColors: Record<string, string> = { INACTIVE: 'default', ACTIVE: 'success', DISABLED: 'error' };
const onlineBadge: Record<string, { status: 'success' | 'default' | 'warning'; text: string }> = {
  ONLINE: { status: 'success', text: '在线' }, OFFLINE: { status: 'default', text: '离线' }, UNKNOWN: { status: 'warning', text: '未知' },
};
...
const downloadDeviceTriples = (records: DeviceCredentialRecord[]) => {
  const header = ['设备ID', '产品ID', '产品Key', '产品名称', '设备名称', '设备别名', '设备密钥'].join(',');
  const rows = records.map((item) => [item.id, item.productId, item.productKey, item.productName || '', item.deviceName, item.nickname || '', item.deviceSecret]
    .map((value) => {
      const text = `${value ?? ''}`;
...
const DeviceList: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await deviceApi.list({ ...params, keyword: keyword || undefined, productId: filterProduct, status: filterStatus, onlineStatus: filterOnline });
  ...
  useEffect(() => { void fetchProducts(); void fetchTags(); }, []);
  useEffect(() => { void fetchData(); }, [params.pageNum, params.pageSize, keyword, filterProduct, filterStatus, filterOnline]);

  const ensureManualRegistrationProducts = () => {
    if (manualProducts.length > 0) return true;
    message.warning('当前没有可手动创建设备的产品，请使用一机一密产品或通过动态注册创建设备');
    return false;
  };

  const resetBatchState = () => { setBatchImportFile(null); setImportProgress(null); setBatchCreating(false); batchForm.resetFields(); };
  ...
  const closeBatchDrawer = () => { setBatchOpen(false); resetBatchState(); };
  ...
  const handleBatchCreate = async (values: DeviceBatchCreateFormValues) => {
    ...
    try {
      ...
      const importRes = await deviceApi.importDevices({ productId: values.productId, fileKey, fileFormat: batchImportFile.name.toLowerCase().endsWith('.csv') ? 'CSV' : 'XLSX', description: values.description, tagIds: values.tagIds });
  ...
  const handleEdit = (record: DeviceRecord) => {
    ...
    editForm.setFieldsValue({ nickname: record.nickname, description: record.description, tagIds: record.tagList?.map((item) => item.id) || [] });
    setEditOpen(true);
  };
  ...
  const handleExportTriples = async () => {
    ...
    try {
      ...
      downloadDeviceTriples(credentials);
  ...
  const columns: ColumnsType<DeviceRecord> = [
    ...
    { title: '标签', dataIndex: 'tagList', width: 240, render: (_: unknown, record: DeviceRecord) => record.tagList?.length ? <Space size={[6, 6]} wrap>{record.tagList.map((item) => <Tag key={item.id} color={item.color || 'blue'}>{item.tagKey}: {item.tagValue}</Tag>)}</Space> : <Typography.Text type="secondary">未设置</Typography.Text> },
    { title: '状态', dataIndex: 'status', width: 90, render: (value: string) => <Tag color={statusColors[value]}>{statusLabels[value] || value}</Tag> },
    { title: '在线', dataIndex: 'onlineStatus', width: 90, render: (value: string) => { const badge = onlineBadge[value] || onlineBadge.UNKNOWN; return <Badge status={badge.status} text={badge.text} />; } },
    ...
    { title: '最近在线', dataIndex: 'lastOnlineAt', width: 180, render: (value?: string) => value || '-' },
  ...
  return (
    <div>
      ...
      <Card>
        <Table rowKey="id" rowSelection={rowSelection} columns={columns} dataSource={data} loading={loading} scroll={{ x: 1600 }} pagination={{ current: params.pageNum, pageSize: params.pageSize, total, showSizeChanger: true, showTotal: (count) => `共 ${count} 条`, onChange: (page, pageSize) => setParams({ pageNum: page, pageSize }) }} />
      </Card>

      <Modal title="新建设备" open={createOpen} width={600} destroyOnHidden onCancel={closeCreateModal} onOk={() => createForm.submit()}>
        <Form form={createForm} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="productId" label="所属产品" rules={[{ required: true, message: '请选择产品' }]}>
            <Select options={manualProducts.map((item) => ({ value: item.id, label: `${item.name} (${item.productKey}) · ${DEVICE_AUTH_LABELS[item.deviceAuthType || 'DEVICE_SECRET'] || '一机一密'}` }))} />
          </Form.Item>
          <Form.Item name="deviceName" label="设备名称" rules={[{ required: true, message: '请输入设备名称' }, { pattern: DEVICE_NAME_PATTERN, message: DEVICE_NAME_RULE_MESSAGE }]}>
            <Input maxLength={64} placeholder="例如：AA:BB:CC:DD:EE:FF / SN20240301001" />
          </Form.Item>
      ...
      <Drawer title="批量导入设备" open={batchOpen} width={760} destroyOnClose onClose={closeBatchDrawer} footer={<Space style={{ width: '100%', justifyContent: 'flex-end' }}><Button onClick={closeBatchDrawer}>取消</Button><Button type="primary" loading={batchCreating} onClick={() => batchForm.submit()}>开始导入</Button></Space>}>
        <Form form={batchForm} layout="vertical" onFinish={handleBatchCreate}>
          <Alert type="warning" showIcon style={{ marginBottom: 16 }} message="Excel/CSV 批量导入仅适用于一机一密产品" description="文件会上传到对象存储并由服务端异步解析，统一标签会直接绑定到本次导入的设备上。" />
          <Form.Item name="productId" label="所属产品" rules={[{ required: true, message: '请选择产品' }]}>
            <Select options={manualProducts.map((item) => ({ value: item.id, label: `${item.name} (${item.productKey}) · ${DEVICE_AUTH_LABELS[item.deviceAuthType || 'DEVICE_SECRET'] || '一机一密'}` }))} />
          </Form.Item>
          ...
          <Form.Item name="tagIds" label="统一设备标签"><Select mode="multiple" allowClear options={tagOptions} optionFilterProp="label" placeholder="可选，统一绑定本次导入设备" /></Form.Item>
          <Form.Item name="description" label="统一描述"><TextArea rows={3} placeholder="可选，为本次导入的全部设备设置统一描述" /></Form.Item>
        </Form>
      </Drawer>

      <Modal title="编辑设备" open={editOpen} width={600} destroyOnHidden onCancel={closeEditModal} onOk={() => editForm.submit()}>
