...
const DeviceShadowPage: React.FC = () => {
  ...
  const overviewItems = [
    ...
    },
    {
      ...
      label: '待同步差异',
      value: `${deltaCount}`,
      hint: deltaCount > 0 ? '设备尚未追平期望状态' : '设备已与期望状态同步',
  ...
  return (
    <div>
      <PageHeader
        title="设备影子"
        description="按设备查看期望值、上报值与差异状态，支持直接维护 desired 并观察同步结果。"
      />
        ...
        <Space direction="vertical" size={18} style={{ width: '100%' }}>
          <Space direction="vertical" size={6} style={{ width: '100%' }}>
            ...
            <Paragraph style={{ margin: 0, color: '#64748b' }}>
              这里优先用设备名称和别名来定位设备，避免手工输入数据库主键。查询后可直接维护 desired，并查看 reported、delta 与 metadata。
            </Paragraph>
          </Space>
          ...
          {selectedDevice ? (
            ...
            <Alert
              ...
              showIcon
              message="还未选择设备"
              description="可按设备名称或别名搜索，选中后再查看影子。"
            />
          )}
        </Space>
      </Card>
          ...
          <Row gutter={[16, 16]}>
            <Col xs={24} xl={12}>
              ...
              >
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  <Paragraph style={{ margin: 0, color: '#64748b' }}>
                    平台希望设备最终达到的状态。保存时会覆盖对应字段，填 `null` 可清除单个属性。
                  </Paragraph>
                  <CodeEditorField
                    language="json"
            ...
            <Col xs={24} xl={12}>
              ...
              >
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  <Paragraph style={{ margin: 0, color: '#64748b' }}>
                    设备当前实际上报的状态。这里为只读区，主要用于和 desired 做差异比对。
                  </Paragraph>
                  <CodeEditorField
                    language="json"
