...
/**
 * SNMP 协议适配器 — 将 SNMP 采集/Trap 数据转为统一 DeviceMessage
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class SnmpProtocolAdapter implements ProtocolAdapter {
    ...
    public boolean supports(String topic) {
        return topic != null && topic.startsWith("/snmp/");
    }
    ...
    public DeviceMessage decode(String topic, byte[] payload, Map<String, String> headers) {
        try {
            ...
            DeviceMessage.MessageType type = topic.contains("/trap/")
            ...
            log.error("Failed to decode SNMP payload: {}", e.getMessage());
            return null;
        }
    }
    ...
    public byte[] encode(DeviceMessage message) {
        try {
            ...
            log.error("Failed to encode SNMP message: {}", e.getMessage());
